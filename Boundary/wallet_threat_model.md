...
## Boundary-Specific Threat Considerations

Boundary introduces new attack vectors through its routing and cross-chain capabilities. This section extends the Zashi threat model with Boundary-specific considerations.

### Malicious Routing Provider Adversary

**Description:** A routing provider (router) is compromised or malicious. The router can see user intents and route discovery requests, but cannot access wallet keys or full wallet state.

**New Security Invariants (Boundary-specific):**
- Routers cannot access user's wallet keys or seed phrases
- Routers cannot access user's transaction history or balances
- Routers cannot link multiple intents to the same user (if addresses are isolated)
- Routers cannot force user to select a malicious route (user must confirm)

**Known Weaknesses (Boundary-specific):**
- **Routers can see user intents and infer user behavior.**
  - Routers receive intent data (assets, amounts, destinations)
  - Routers can correlate intents if addresses are reused
  - Routers can infer user's asset preferences and transaction patterns
- **Routers can suggest malicious routes.**
  - Routers can suggest routes that benefit the router or its partners
  - Routers can suggest routes with hidden fees or unfavorable exchange rates
  - Users must validate routes before execution (client-side validation required)
- **Routers can censor route discovery.**
  - Routers can omit certain routes (e.g., privacy-preserving routes)
  - Routers can prioritize routes that benefit the router
  - Users should use multiple routers to avoid censorship

**Mitigations:**
- Client-side route validation (verify route steps are executable)
- Multiple router support (compare routes from different routers)
- Privacy-first routing (prefer routes that don't leak metadata)
- User confirmation required (users must approve route selection)

### Malicious Bridge/Execution Provider Adversary

**Description:** A bridge or execution provider is compromised or malicious. The provider can execute route steps but cannot access wallet keys (signing happens client-side).

**New Security Invariants (Boundary-specific):**
- Execution providers cannot access user's wallet keys (signing happens client-side)
- Execution providers cannot steal user's funds (non-custodial providers)
- Execution providers cannot execute unauthorized transactions (user must sign)

**Known Weaknesses (Boundary-specific):**
- **Custodial bridges can steal or freeze user's funds.**
  - Custodial bridges hold user's funds temporarily
  - Malicious custodial bridges can steal funds or refuse to release them
  - Users should prefer non-custodial bridges when available
- **Execution providers can fail to execute routes.**
  - Providers can fail mid-execution (partial failures)
  - Providers can refuse to execute certain routes
  - Retry logic and rollback mechanisms required
- **Execution providers can leak transaction metadata.**
  - Providers can see transaction data for their step
  - Providers can correlate transactions if addresses are reused
  - Privacy-preserving providers preferred, but not always available

**Mitigations:**
- Prefer non-custodial execution providers
- Client-side transaction signing (keys never leave device)
- Retry logic and rollback mechanisms
- Privacy-preserving providers when available

### Cross-Chain Routing Adversary

**Description:** An adversary who can observe or interfere with cross-chain routing. This includes bridge operators, network observers, and routing providers.

**New Security Invariants (Boundary-specific):**
- Cross-chain routes are validated client-side before execution
- Users can see all route steps before confirming
- Partial failures trigger rollback mechanisms (where possible)

**Known Weaknesses (Boundary-specific):**
- **Cross-chain routes have multiple failure points.**
  - Each step in a multi-hop route can fail independently
  - Partial failures can leave funds in intermediate states
  - Rollback may not always be possible (depends on bridge protocol)
- **Bridge operators can observe cross-chain transactions.**
  - Bridge operators see all transactions through their bridge
  - Bridge operators can correlate transactions if addresses are reused
  - Privacy-preserving bridges preferred, but not always available
- **Network observers can correlate cross-chain transactions.**
  - Timing analysis can link transactions across chains
  - Address reuse can link transactions to the same user
  - Privacy-preserving routing and address isolation mitigate this

**Mitigations:**
- Multi-step route validation (verify all steps before execution)
- Privacy-preserving bridges when available
- Address isolation (generate new addresses per intent)
- Clear error messages and rollback instructions

### Privacy Leakage Through Routing

**Description:** An adversary who can observe routing behavior and infer user preferences or transaction patterns.

**New Security Invariants (Boundary-specific):**
- Routers receive only intent data (no wallet state)
- Privacy scores are calculated and displayed to users
- Privacy-first routing by default

**Known Weaknesses (Boundary-specific):**
- **Routers can infer user preferences from routing requests.**
  - Frequent requests for certain assets reveal user preferences
  - Routing patterns can reveal user's transaction frequency
  - Address reuse can link multiple intents to the same user
- **Route selection reveals user's privacy preferences.**
  - Users who always select HIGH privacy routes reveal privacy-conscious behavior
  - Users who select LOW privacy routes reveal cost-conscious behavior
  - Privacy preferences are visible to routers
- **Cross-chain routing reveals user's multi-chain activity.**
  - Cross-chain intents reveal user holds assets on multiple chains
  - Routing patterns can reveal user's chain preferences
  - Bridge usage reveals user's cross-chain activity

**Mitigations:**
- Data minimization (routers receive only intent data)
- Privacy-first routing (default HIGH privacy)
- Address isolation (generate new addresses per intent)
- Multiple router support (distribute routing requests)
...
- Routers cannot see user's transaction history or balances

### Execution Privacy
- Execution providers receive only the `RouteStep` they need to execute
- Execution providers cannot see other route steps or wallet state
- Signing happens client-side (keys never leave device)

### Route Privacy
- Privacy scores are calculated and displayed for all routes
- Privacy-first routing by default (HIGH privacy preferred)
- Users can see privacy impact of route selection

### Limitations
- Privacy depends on blockchain capabilities (shielded vs. transparent)
- Bridge operators may see transactions (custodial bridges)
- Network observers may see routing requests (HTTPS metadata)
- Address reuse can link transactions (mitigated by address isolation)

---

## Recommendations for Boundary Users

1. **Use privacy-first routing** (default setting)
2. **Prefer non-custodial bridges** when available
3. **Use multiple routers** to avoid censorship
4. **Generate new addresses per intent** (address isolation)
5. **Review route steps before confirming** (client-side validation)
6. **Use VPN/Tor** for network privacy (if needed)
7. **Understand privacy trade-offs** (HIGH privacy vs. cost/latency)

---

**Note:** This threat model will be updated as Boundary's routing and cross-chain capabilities evolve. See [Privacy Routing](./docs/privacy_routing.md) for detailed privacy constraints.
